...
(function() {
  var client, express, http, log, polar, setup_app, setup_io, socketio, somata, util,
    slice = [].slice;
  ...
  setup_app = function(options) {
    var app, base_app, http_server, io;
    base_app = express();
    ...
    return io.on('connection', function(socket) {
      ...
      socket.on('remote', function() {
        ...
        return client.remote.apply(client, [service, method].concat(slice.call(args), [function(err, data) {
          console.log('[client.remote] ' + util.inspect(arguments, {
            colors: true
          }));
          return cb(err, data);
        }]));
      ...
      socket.on('subscribe', function(service, type) {
        ...
        handler = client.on(service, type, function(event) {
        ...
        return subscriptions[service][type].push(handler);
      });
